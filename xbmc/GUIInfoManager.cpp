...
const int picture_slide_map[]  = {/* LISTITEM_PICTURE_RESOLUTION => */ SLIDESHOW_RESOLUTION,
...
std::string CGUIInfoManager::GetLabel(int info, int contextWindow, std::string *fallback)
{
  ...
  if (info >= SLIDESHOW_INFO_START && info <= SLIDESHOW_INFO_END)
...
bool CGUIInfoManager::GetBool(int condition1, int contextWindow, const CGUIListItem *item)
{
  ...
  else if (condition == SYSTEM_HAS_PVR_ADDON)
  {
    ...
    bReturn = CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_showExitButton;
